...
```typescript
function cancel(params: { modelId: string; operation: "inference" } | { clearCache?: boolean; delegate?: { healthCheckTimeout?: number; providerPublicKey: string; timeout?: number; topic: string }; downloadKey: string; operation: "downloadAsset" } | { operation: "rag"; workspace?: string } | { modelId: string; operation: "embeddings" }): Promise
```
...
| --- | --- | :---: | --- |
| `params` | `\{ modelId: string; operation: "inference" \} | \{ clearCache?: boolean; delegate?: \{ healthCheckTimeout?: number; providerPublicKey: string; timeout?: number; topic: string \}; downloadKey: string; operation: "downloadAsset" \} | \{ operation: "rag"; workspace?: string \} | \{ modelId: string; operation: "embeddings" \}` | ✓ | The parameters for the cancellation |
